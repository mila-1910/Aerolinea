...
// ============================================================
// NUEVO ENDPOINT DE REPORTES (AGENTE)
// ============================================================

app.get('/api/agente/reportes', async (req, res) => {
    try {
        ...
        const reservasPorVueloRes = await pool.query(`
            SELECT
                v.cod_vuelo,
                TO_CHAR(r.fecha_hora_reserva, 'YYYY-MM') AS mes,
                COUNT(*)::INT AS total_reservas
        ...
        const clientesFrecuentesRes = await pool.query(`
            SELECT
                c.numero_identificacion,
                c.nombres || ' ' || c.apellidos AS cliente,
                COUNT(*)::INT AS total_reservas
        ...
        const vuelosGeografiaRes = await pool.query(`
            SELECT
                v.cod_vuelo,
        ...
        const destinosVendidosRes = await pool.query(`
            SELECT
                p.nombre_pais AS pais,
        ...
        const tiempoPromedioRes = await pool.query(`
            SELECT
        ...
        const historialClientesRes = await pool.query(`
            SELECT
                c.numero_identificacion,
                c.nombres || ' ' || c.apellidos AS cliente,
                r.id_reserva,
        ...
        const canceladasCausaRes = await pool.query(`
            SELECT
                r.id_reserva,
                c.nombres || ' ' || c.apellidos AS cliente,
                v.cod_vuelo,
        ...
        res.json({
            ingresosPorDestino: ingresosRes.rows,
            reservasPorVuelo: reservasPorVueloRes.rows,
            clientesFrecuentes: clientesFrecuentesRes.rows,
            vuelosPorGeografia: vuelosGeografiaRes.rows,
            destinosMasVendidos: destinosVendidosRes.rows,
            tiempoPromedioConfirmacion: tiempoPromedioRes.rows[0]?.promedio_minutos || 0,
            historialClientes: historialClientesRes.rows,
            canceladasConCausa: canceladasCausaRes.rows
        });
    } catch (error) {
        console.error('Error al obtener reportes del agente:', error);
        res.status(500).json({ error: 'Error al obtener reportes del agente' });
    }
});
